...
FROM 'C:\Users\Heino\Documents\Code Work\Postgres SQL\Practical SQL\Chapter_4\practical-sql-main\Chapter_13\crime_reports.csv'
...
--  Using regexp_match() to find the second date Note that the result includes an unwanted hyphen
 
SELECT crime_id,
       regexp_match(original_text, '-\d{1,2}\/\d{1,2}\/\d{1,2}')
FROM crime_reports;

-- Using a capture group to return only the date Eliminates the hyphen
SELECT crime_id,
       regexp_match(original_text, '-(\d{1,2}\/\d{1,2}\/\d{1,2})')
FROM crime_reports;
...
FROM 'C:\Users\Heino\Documents\Code Work\Postgres SQL\Practical SQL\Chapter_4\practical-sql-main\Chapter_13\sotu-1946-1977.csv'
